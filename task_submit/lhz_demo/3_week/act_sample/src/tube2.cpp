              ...
              for (j=0;j<width;j++)
               {
                 ...
                 //temp=(gama*(R+G+B)-MaxC)/(3*gama-1);
                dstData[j*3]=R-(unsigned char)(temp+0.9);
                dstData[j*3+1]=G-(unsigned char)(temp+0.9);
                dstData[j*3+2]=B-(unsigned char)(temp+0.9);   
        }
    } 
    return 1;
}
...
{
    Mat srcImg=cvarrToMat(src_img, true);
    ...
    //IplImage src_img1 = dstImg.operator IplImage();
    IplImage* src_img1 ;
    *src_img1= IplImage(dstImg);
     src_img= cvCloneImage(src_img1);
}
