...
int main()
{
	IplImage * src_img=cvLoadImage("sim_sample8.jpg");
    if(!src_img)
    {
    ...
    IplImage * src = cvLoadImage("sim_sample8.jpg");
    ...
    for (int i = 0; i < height; i++)
        for (int j = 0; j < width; j++)
        {
            ...
            CvScalar s;
            if (!(((s_src_img.val[0]>0)&&(s_src_img.val[0]<8)) || (s_src_img.val[0]>120)&&(s_src_img.val[0]<180)))
            {
                s.val[0] =0;
                s.val[1]=0;
                s.val[2]=0;
    ...
    cvThreshold(gray_img, adp_img, 32, 255.0, CV_THRESH_BINARY); 
    //二值化图片
    ...
    cvDilate(xtx_img, xtx_img, kernal, 6);
    cvErode(xtx_img, xtx_img, kernal, 7);   //y 腐蚀去除碎片      
    //自定义 3x1 的核进行 y 方向的膨胀腐蚀
    kernal = cvCreateStructuringElementEx(3, 1, 1, 0, CV_SHAPE_RECT);
    cvDilate(xtx_img, xtx_img, kernal, 6);   //x 膨胀回复形态
    cvErode(xtx_img, xtx_img, kernal, 7);    //x 腐蚀去除碎片 
