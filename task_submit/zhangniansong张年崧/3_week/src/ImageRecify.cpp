...
void moveImage(Mat src, Mat& img_moved, int x, int y)
{
    ...
    warpAffine(src, img_moved, t_mat,  Size(src.cols, src.rows), 1, 0, Scalar(255, 255, 255));
}
...
void ImageRecify(Mat src, Mat& resulyImage)
{
    ...
    if (degree == ERROR)
    {
        cout << "Calculate degree failed" << endl;
        resulyImage = src;
    }
    ...
    resulyImage = dst(Rect(0, 0, srcWidth, srcHight)); //根据先验知识，估计好长宽，再裁剪下来
